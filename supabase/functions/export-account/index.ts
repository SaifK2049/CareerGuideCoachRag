...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const queries = await Promise.all([
      ...
      userClient.from("career_graph_nodes").select("*").eq("user_id", userId).order("created_at"),
      userClient.from("career_graph_edges").select("*").eq("user_id", userId).order("created_at"),
      userClient.from("career_twins").select("*").eq("user_id", userId).maybeSingle(),
      userClient.from("career_twin_snapshots").select("*").eq("user_id", userId).order("created_at"),
      userClient.from("career_recommendations").select("*").eq("user_id", userId).order("generated_at"),
      userClient.from("organization_data_consents").select("*").eq("user_id", userId).order("created_at"),
      userClient.from("organization_memberships").select("*").eq("user_id", userId).order("created_at"),
      userClient.from("organization_cohort_members").select("*").eq("user_id", userId).order("created_at"),
      userClient.from("mentor_profiles").select("*").eq("user_id", userId).maybeSingle(),
      userClient.from("mentor_matches").select("*").eq("user_id", userId).order("generated_at"),
      userClient.from("mentorship_requests").select("*").eq("user_id", userId).order("requested_at"),
      userClient.from("company_intelligence_briefs").select("*").eq("user_id", userId).order("generated_at"),
      userClient.from("interview_video_assessments").select("*").eq("user_id", userId).order("created_at"),
      userClient.from("portfolio_assets").select("*").eq("user_id", userId).order("created_at"),
      userClient.from("career_credentials").select("*").eq("user_id", userId).order("created_at"),
      userClient.from("career_readiness_assessments").select("*").eq("user_id", userId).order("created_at"),
      userClient.from("career_simulations").select("*").eq("user_id", userId).order("created_at"),
      userClient.from("learning_roadmaps").select("*").eq("user_id", userId).order("created_at"),
      userClient.from("learning_roadmap_milestones").select("*").eq("user_id", userId).order("created_at"),
    ...
    return jsonResponse(request, {
      ...
      career_graph_nodes: queries[15].data || [],
      career_graph_edges: queries[16].data || [],
      career_twin: queries[17].data,
      career_twin_snapshots: queries[18].data || [],
      career_recommendations: queries[19].data || [],
      organization_data_consents: queries[20].data || [],
      organization_memberships: queries[21].data || [],
      organization_cohort_memberships: queries[22].data || [],
      mentor_profile: queries[23].data,
      mentor_matches: queries[24].data || [],
      mentorship_requests: queries[25].data || [],
      company_intelligence_briefs: queries[26].data || [],
      interview_video_assessments: queries[27].data || [],
      portfolio_assets: queries[28].data || [],
      career_credentials: queries[29].data || [],
      career_readiness_assessments: queries[30].data || [],
      career_simulations: queries[31].data || [],
      learning_roadmaps: queries[32].data || [],
      learning_roadmap_milestones: queries[33].data || [],
      product_events: queries[34].data || [],
      stored_cv_files: (queries[35].data || []).map((file) => ({
        name: file.name,
